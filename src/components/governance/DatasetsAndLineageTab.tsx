        ...
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-r from-teal-900 to-slate-900 text-white p-4 rounded-2xl border border-teal-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-sm">
            <div className="flex items-center gap-2.5 text-xs">
              ...
              <div>
                <p className="font-bold">Moteur de Traçabilité Ascendante de Provenance (Data Lineage V1.19)</p>
                    ...
                    <React.Fragment key={node.id}>
                      ...
                      >
                        <div>
                          ...
                          <h5 className="text-xs font-bold mt-2 line-clamp-2">
                            {node.label}
                          </h5>
                        </div>

                        <div className="pt-2 border-t border-black/10 flex items-center justify-between text-[10px] text-slate-600">
              ...
              <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200/80 space-y-3 text-xs">
                <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 pb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-sm text-slate-900">{selectedNode.label}</span>
                    <span className="text-xs font-semibold px-2 py-0.5 rounded bg-emerald-100 text-emerald-800">
                      {selectedNode.validationStatus}
                    </span>
                  </div>
                  <span className="text-slate-500 font-mono">Horodatage : {selectedNode.date}</span>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <span className="text-slate-400 font-semibold uppercase text-[10px]">Description & Rôle :</span>
                    <p className="text-slate-800 font-medium mt-0.5">{selectedNode.details}</p>
                  </div>

                  <div>
                    <span className="text-slate-400 font-semibold uppercase text-[10px]">Règles de Transformation Appliquées :</span>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-xs space-y-6">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-100">
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-xl bg-teal-700 hover:bg-teal-800 text-white text-xs font-semibold flex items-center justify-center gap-1.5 shadow-xs transition-all shrink-0"
                >
              ...
              <div className="space-y-3">
                <div className="p-3 bg-slate-50 rounded-xl border border-slate-200/80 font-mono text-xs space-y-1">
                ...
                <div className="space-y-1 text-xs">
                  ...
                  <ul className="list-disc list-inside space-y-1 text-slate-700 bg-slate-50 p-3 rounded-xl border border-slate-200/60">
              ...
              <div className="space-y-3">
                ...
                {selectedDataset.snapshots.length === 0 ? (
                  <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 text-slate-400 text-xs text-center">
                  ...
                  <div className="space-y-3">
                    {selectedDataset.snapshots.map((snp) => (
                      <div
                        key={snp.snapshotId}
                        className="p-4 rounded-xl bg-gradient-to-r from-teal-50/50 to-white border border-teal-200 space-y-2 text-xs"
                      >
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          <div className="flex items-center gap-2">
                            <span className="font-mono font-bold text-teal-900 bg-teal-100 px-2 py-0.5 rounded">
                              {snp.snapshotId}
                            </span>
                          ...
                          <span className="text-slate-500 font-mono text-[11px]">{snp.createdAt}</span>
                        </div>

                        <p className="text-slate-700">{snp.notes}</p>

                        <div className="pt-2 border-t border-teal-100 flex flex-wrap items-center justify-between gap-2 text-[11px] font-mono text-slate-600">
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl border border-slate-200 max-w-lg w-full p-6 shadow-2xl space-y-4 animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="space-y-3 text-xs">
              ...
              <div>
                <label className="block font-semibold text-slate-700 mb-1">Nom du Snapshot (ex: Clôture T3-2026)</label>
                <input
                  type="text"
                  value={snapshotName}
              ...
              <div>
                <label className="block font-semibold text-slate-700 mb-1">Notes et Justification d Archivage</label>
                <textarea
                  rows={3}
                  value={snapshotNotes}
